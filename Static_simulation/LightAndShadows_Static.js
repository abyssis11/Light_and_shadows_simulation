...
// takodjer oznacujemo vrhove s krugom (drawCircle())
function createRect(size){
    ...
    drawCircle(x + size, y, radius);
    drawCircle(x + size, y + size, radius);
    drawCircle(x, y + size, radius);
...
// Stvaramo N kvadrata i projeciramo zrake prema svim njihovim vrhovima i provjeravamo koliziju na njihovom putu
// vracamo listu krajnjih tocaka svake zrake te njihov nagib 
function rays(centerX, centerY, size, d, accumulator, N){

    // kad smo stvorili N kvadrata i saznali sve njihove vrhove, projeciramo zrake prema njima i provjeravamo koliziju na njihovom putu
    if(N===0){
    ...
    const linesToEdge = [
        ...
        drawLine(centerX, centerY, centerX*2, 0, accumulator, size),
        drawLine(centerX, centerY, centerX*2, centerY*2, accumulator, size),
        drawLine(centerX, centerY, 0, centerY*2, accumulator, size)
    ]
    ...
    // pozivamo funkciju endge() koja nam vraca sve vrhove za stvoreni kvadrat
    const rect = createRect(size);
    return rays(centerX, centerY, size, d, accumulator.concat(rect), N-1)          
}
...
const projectedRays = rays(canvas.width/2, canvas.height/2, sizeOfrects, diagonal, [], Nrects);
